					...
					<table class="w-full table-fixed">
						<thead>
							<tr class="border-b border-gray-100 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
								<th class="px-5 py-3 w-[30%]">Title</th>
								<th class="px-3 py-3 w-[10%]">Status</th>
								<th class="px-3 py-3 w-[14%]">Proposer</th>
								<th class="px-3 py-3 w-[10%]">Created</th>
								<th class="px-3 py-3 w-[14%]">Votes</th>
								<th class="px-3 py-3 w-[22%]">Actions</th>
							</tr>
						</thead>
						<tbody class="divide-y divide-gray-100">
							{#each pagedProposals as proposal}
								<tr class="hover:bg-gray-50 transition-colors">
									<td class="px-5 py-3">
										<div class="truncate">
											<div class="text-sm font-medium text-gray-900 truncate">{proposal.title}</div>
											<div class="text-xs text-gray-500 truncate">{proposal.description}</div>
										</div>
									</td>
									<td class="px-3 py-3">
									...
									<td class="px-3 py-3 truncate" title={proposal.proposer}>
										<code class="text-xs text-gray-500">{truncatePrincipal(proposal.proposer)}</code>
									</td>
									<td class="px-3 py-3 text-xs text-gray-500">{formatDate(proposal.created_at)}</td>
									<td class="px-3 py-3">
										{#if proposal.status === 'voting'}
											<div class="text-xs space-y-1">
												...
												<div class="w-full bg-gray-200 rounded-full h-1">
													<div class="bg-green-500 h-1 rounded-full" style="width: {((proposal.votes?.yes ?? 0) / Math.max(proposal.total_voters, 1)) * 100}%"></div>
												</div>
											</div>
										{:else}
									...
									<td class="px-3 py-3">
										<div class="flex flex-wrap gap-1">
											<button
												...
												class="inline-flex items-center gap-1 px-2.5 py-1 rounded border border-gray-300 text-xs text-gray-700 hover:bg-gray-100 transition-colors"
											>
											...
											{#if proposal.status === 'voting'}
												<button
													...
													disabled={!!votingInProgress}
													class="px-2 py-1 rounded border border-green-300 text-xs text-green-700 hover:bg-green-50 disabled:opacity-50"
												>
													{#if votingInProgress === proposal.id + 'yes'}
														<div class="w-3 h-3 border-2 border-green-400 border-t-transparent rounded-full animate-spin inline-block"></div>
													{:else}Yes{/if}
												</button>
												<button
													onclick={() => castVote(proposal.id, 'no')}
													disabled={!!votingInProgress}
													class="px-2 py-1 rounded border border-red-300 text-xs text-red-700 hover:bg-red-50 disabled:opacity-50"
												>
													{#if votingInProgress === proposal.id + 'no'}
														<div class="w-3 h-3 border-2 border-red-400 border-t-transparent rounded-full animate-spin inline-block"></div>
													{:else}No{/if}
												</button>
												<button
													onclick={() => castVote(proposal.id, 'abstain')}
													disabled={!!votingInProgress}
													class="px-2 py-1 rounded border border-gray-300 text-xs text-gray-600 hover:bg-gray-50 disabled:opacity-50"
												>
													{#if votingInProgress === proposal.id + 'abstain'}
														<div class="w-3 h-3 border-2 border-gray-400 border-t-transparent rounded-full animate-spin inline-block"></div>
													{:else}Abstain{/if}
												</button>
											{/if}
										</div>

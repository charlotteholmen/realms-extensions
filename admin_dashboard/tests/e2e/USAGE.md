...
```bash
E2E_WARMUP_PATH="/admin" ./run-e2e-tests.sh
```

### Debug Mode
```bash
E2E_WARMUP_PATH="/admin" ./run-e2e-tests.sh --debug
```

### Headed Mode
```bash
E2E_WARMUP_PATH="/admin" ./run-e2e-tests.sh --headed
```

## Environment Variables

- `PLAYWRIGHT_BASE_URL` - Base URL for tests (default: `http://localhost:8000`)
- `E2E_WARMUP_PATH` - Path to warm up before tests (recommended: `/admin`)
